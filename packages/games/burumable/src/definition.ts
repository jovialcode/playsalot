...
function finishTurn(state: BurumableState) { nextPlayer(state); state.phase = "roll"; state.lastRoll = 0; state.message = `${playerName(state, state.turnIndex)}님의 차례: 주사위를 굴리세요.`; }
function resolveLanding(state: BurumableState, playerIndex: number) {
  ...
  if (space === 0) { state.cash[playerIndex] = money + 200; state.message = "출발점을 지나 보너스 ₩200을 받았어요!"; finishTurn(state); return; }
  if (space === 3 || space === 9 || space === 15) { const bonus = 120; state.cash[playerIndex] = money + bonus; state.message = `황금열쇠! ₩${bonus} 보너스를 받았어요.`; finishTurn(state); return; }
  if (space === 6) { state.message = "휴식 공간입니다. 한 턴 쉬어가세요."; finishTurn(state); return; }
  if (space === 12) { const penalty = 100; state.cash[playerIndex] = money - penalty; state.message = `무인도 탈출 비용 ₩${penalty}!`; if (money - penalty <= 0) bankrupt(state, playerIndex); else finishTurn(state); return; }
  if (space === 18) { state.positions[playerIndex] = 0; state.cash[playerIndex] = money + 200; state.message = "세계여행! 출발점으로 이동해 ₩200을 받았어요."; finishTurn(state); return; }
  ...
  const cost = rent(space, state.levels[space] ?? 0); state.cash[playerIndex] = money - cost; state.cash[owner] = (state.cash[owner] ?? 0) + cost; state.message = `${SPACES[space] ?? "도시"} 통행료 ₩${cost}을 냈어요.`; if (money - cost <= 0) bankrupt(state, playerIndex); else finishTurn(state);
}
...
export const burumableDefinition: GameDefinition<BurumableState, BurumableMove> = {
  ...
  applyMove(state, playerId, move): GameMoveResult {
    ...
    if (!move || !["roll", "buy", "pass", "build"].includes(move.action)) return { ok: false, error: "알 수 없는 행동이에요." };
