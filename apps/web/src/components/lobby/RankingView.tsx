...
import type { GuestSession, Leaderboard } from "@playsalot/shared-types";
...
export function RankingView({ session, onGoLogin }: RankingViewProps) {
  const [rows, setRows] = useState<Row[] | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!session || !isMember(session)) return;
    friendRequest<Leaderboard>(session, "/ranking")
      .then((board) => {
        const list: Row[] = board.entries.map((entry) => ({
          name: entry.displayName,
          score: entry.score,
          rank: entry.rank,
          isMe: entry.userId === session.guestId,
        }));
        // Show the caller even if they rank below the leaderboard cutoff.
        if (board.me && !list.some((row) => row.isMe)) {
          list.push({ name: board.me.displayName, score: board.me.score, rank: board.me.rank, isMe: true });
        }
        setRows(list);
      })
  ...
  if (!isMember(session)) {
    return <LoginRequired feature="랭킹" title="랭킹" eyebrow="리더보드" onGoLogin={onGoLogin} />;
  }
  ...
  if (error || rows === null || rows.length === 0) {
    const message = error
      ...
      : rows === null
        ? "랭킹을 불러오는 중이에요."
        : "이번 달 랭킹 기록이 아직 없어요. 사람과 대전해서 첫 점수를 올려보세요.";
    return (
      <div className="flex flex-col">
        {header}
        <div className="p-[40px_20px] text-center text-[13px] text-[var(--ink-mute)]">{message}</div>
      </div>
    );
  }

  const podium = rows.slice(0, 3);
  const remainingRanks = rows.slice(3);
  // Center the champion: [2nd, 1st, 3rd]. Any missing slot is dropped.
  const podiumOrder = [podium[1], podium[0], podium[2]].filter((p): p is Row => Boolean(p));
      ...
      <div className="flex flex-col gap-[8px] p-[16px_20px_20px]">
        <section className="stagger-in overflow-hidden rounded-[22px] border border-[var(--mist)] bg-[var(--paper)] p-[18px_12px_12px] shadow-[var(--shadow-2)]" aria-label="상위 3명">
          <div className="mb-[14px] flex items-center justify-between px-[6px]">
            <div>
              <div className="text-[10px] font-bold tracking-[0.12em] text-[var(--coral)]">MONTHLY HIGHLIGHTS</div>
              <div className="mt-[2px] text-[16px] font-bold tracking-[-0.02em] text-[var(--ink)]">이번 달의 챔피언</div>
            </div>
          ...
          <div className="flex items-end justify-center gap-[6px]">
            {podiumOrder.map((p, i) => {
              const style = podiumStyle[p.rank as 1 | 2 | 3] ?? podiumStyle[3];
              return (
                <div key={p.name + p.rank} className={`flex min-w-0 flex-1 flex-col items-center ${p.rank === 1 ? "-translate-y-[6px]" : ""}`} style={{ animationDelay: `${i * 75}ms` }}>
        ...
        {remainingRanks.map((p, i) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0 text-[14px] font-medium text-[var(--ink)]">{p.name}{p.isMe && <span className="ml-[6px] text-[11px] font-semibold text-[var(--sage)]">나</span>}</div>
